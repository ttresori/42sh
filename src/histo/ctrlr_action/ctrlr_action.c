...
static void to_exit_ctrlr(t_42sh *sh, char *dup, char *arg, int is_find)
{
            is_find == 0 \
...
void       ctrlr_read(t_42sh *sh, char *dup, char *arg, int is_find)
{
    int save;
    ...
    while (42)
    {
        ...
        {
            to_exit_ctrlr(sh, dup, arg, is_find);
            return ;
        }
        if (dup)
            free(dup);
        ...
        if (save == sh->history_mark->pos_arg)
